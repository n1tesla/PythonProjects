                ...
                for scaler_type in config.scaler:
                    for val_ratio in config.validation_ratio:
                        for batch_size in config.batch_size:
                            ...
                            preparation=PREPARATION(config,window_size,stride_size,scaler_type,val_ratio)
                            dataset_dict,df_train,df_test,scaler=preparation.create_dataset(data_path)
                            regressor=DeepRegressor(dataset_dict,config,models_dir,plots_dir,observation_name,start_time,parameters_dict,scaler,df_test)
                            regressor.fit()
