...
class ConcereteComponent(AbstractComponent):
    def execute(self,upper_bound:int) ->int:
        count=0
...
def main()->None:
    ...
    component=ConcereteComponent()
    #logging_decorator=LoggingDecorator(component) #if you want to benchmark logging as well, give logging_decorator as argument to benchmark
    benchmark_decorator=BenchMarkDecorator(component)
    logging_decorator = LoggingDecorator(benchmark_decorator)
    value=logging_decorator.execute(100000) #ConcreteComponentin calisma süresini hesaplayacak
